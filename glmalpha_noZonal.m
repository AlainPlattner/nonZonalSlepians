function [G,V]=glmalpha_noZonal(TH,L)
  ...
  dirname=fullfile(getenv('IFILES'),'GLMALPHANZ');
  if lp
    fnpl=fullfile(dirname,sprintf(...
         'GLMALPHANZ-%g-%i.mat',TH,L));
  elseif bp
    fnpl=fullfile(dirname,sprintf(...
         'GLMALPHANZ-%g-%i-%i.mat',TH,L(1),L(2)));
  else
  ...
  if exist(fnpl,'file')==2
    load(fnpl)
    disp(sprintf('%s loaded by glmalpha_noZonal',fnpl))
  else
    % Calculate
